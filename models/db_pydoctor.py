...
tsv_with_hidden_cols=
    (ExporterTSV, 'TSV (Compatible con Excel, columnas ocultas)'),
...
db.define_table('persona',
                Field('sexo', length=2,
                      requires=IS_IN_SET({'F': 'Femenino',
                                          'M': 'Masculino',
                                          'J': 'Persona Jurídica'},
                                         zero=T('Elija persona jurídica o sexo'),
                                         error_message='debe elegir persona jurídica o sexo persona física')),
                Field('apellido', length=70, required=True,
                      label=T('Apellido o Razón Social')),
                Field('nombre', length=100, label=T('Nombre')),
                Field('cuitcuil', length=20, required=True,
                      label=T('CUIT/CUIL')),
                Field('domicilio', length=150, required=True,
                      label=T('Domicilio')),
                Field('email', length=150, requires=IS_EMPTY_OR(IS_EMAIL()),
                      label=T('E-Mail')),
                Field('observaciones', 'text', length=65535,
                ...
                Field('fotografia', 'upload', length=65536,
                      requires=IS_EMPTY_OR(IS_IMAGE())),
                Field('matricula', length=10, label=T('Matrícula'),
                      comment=T('Matrícula profesional de abogado')),
                Field('domiciliolegal', length=150, label=T('Domicilio Legal'),
                      comment=T('Domicilio legal del abogado')),
                auth.signature, format=persona_format)
...
db.juzgado.instancia_id.requires = IS_IN_DB(
    db, db.instancia.id, '%(descripcion)s')
...
db.define_table('tipoproceso',
                Field('descripcion', length=150, required=True,
                      label=T('Descripción')),
                format='%(descripcion)s',
...
def expediente_numero(value, row):
    ...
    url = URL('expedientes', 'index',
              args=['expediente', 'edit', 'expediente', row.id],
              user_signature=True)
...
db.define_table('expediente',
                ...
                Field('caratula', length=200, required=True,
                      label=T('Carátula')),
...
def agenda_titulo(value, row):
    ...
    if row.vencimiento:
        url = URL('agenda', 'calendar',
                  args=[row.vencimiento.strftime('%Y-%m-%d')])
...
def changed_expediente(set_, number):
    if number:
        ...
        for r in set_.select():
            db(db.expediente.id == r.expediente_id).update(
                changed_at=request.now)

db.movimiento._after_insert.append(
    lambda f, id: changed_expediente(None, f['expediente_id']))
db.movimiento._after_update.append(
    lambda s, f: changed_expediente(s, None))
db.movimiento._before_delete.append(
    lambda s: changed_expediente(s, None))
db.agenda._after_insert.append(
    lambda f, id: changed_expediente(None, f['expediente_id']))
db.agenda._after_update.append(
    lambda s, f: changed_expediente(s, None))
db.agenda._before_delete.append(
    lambda s: changed_expediente(s, None))
db.parte._after_insert.append(
    lambda f, id: changed_expediente(None, f['expediente_id']))
db.parte._after_update.append(
    lambda s, f: changed_expediente(s, None))
db.parte._before_delete.append(
    lambda s: changed_expediente(s, None))
