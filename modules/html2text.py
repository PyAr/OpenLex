...
def dumb_property_dict(style):
    ...
    return dict([(x.strip(), y.strip()) for x, y in [z.split(':', 1)
                                                     for z in style.split(';') if ':' in z]])
...
def dumb_css_parser(data):
    ...
    # parse the css. reverted from dictionary compehension in order to support
    # older pythons
...
class _html2text(HTMLParser.HTMLParser):
    ...
    def o(self, data, puredata=0, force=0):
        ...
        if not self.quiet:
            if options.google_doc:
                # prevent white space immediately after 'begin emphasis' marks
                # ('**' and '_')
...
if __name__ == '':  # "__main__":
    ...
    if len(args) > 0:
        ...
        #data = sys.stdin.read()
        data = "<p>&iexcl;Ah! <em>&Ntilde;oqui </em>Es un verdadero <strong>plomo</strong></p>"
    wrapwrite(html2text(data, baseurl))


if __name__ == "__main__":
    data = "<p>&iexcl;Ah! <em>&Ntilde;oqui </em>Es un verdadero <strong>plomo</strong></p>"
    sys.stdout.write(html2text(data))
